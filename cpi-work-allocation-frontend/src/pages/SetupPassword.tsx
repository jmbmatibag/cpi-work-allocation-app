...
const SetupPassword = () => {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Brief pause so the toast registers before the route change.
      setTimeout(() => navigate("/login", { replace: true }), 400);
    } catch (err) {
      const msg =
        err instanceof ApiError
          ? (err.body as { error?: string })?.error ?? "Could not set your password."
  ...
  // ── Invalid / missing token short-circuit ──────────────────────────────
  // Render a clear error state instead of letting the user fill out a form
  // that the server is guaranteed to reject. The setup link is the only
  // way into this page, so a missing token means a hand-typed URL.
  if (!token) {
    return (
        ...
        <div className="relative w-full max-w-md mx-4 text-center">
          <div className="glass-card rounded-2xl p-8 space-y-4">
            ...
            <p className="text-sm text-muted-foreground">
              ...
              welcome email. If you've lost the email, ask an administrator
              to re-send your invitation.
            </p>
